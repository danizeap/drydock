...
**Draining a backlog of stalled packets:** run `python3 scripts/sdd.py triage` (read-only) to bucket every active packet — ARCHIVE-READY / NEEDS-SYNC / CLAIMED-DONE-UNVERIFIED / IN-PROGRESS / UNKNOWN — each with its next action. Archive the ready ones, sync the rest. For a packet that was genuinely **abandoned mid-lifecycle** (real work, never verified, not worth verifying now), use `python3 scripts/sdd.py archive <name> --abandon --reason "<why>"`: it records an honest `Abandoned — never verified` Result (never a synthesized PASS), logs an Override, warns if it buries unsynced spec knowledge, and moves the packet to `archive/`. `--abandon` is distinct from `--force`: use `--force` to waive a specific gate on work you stand behind; use `--abandon` to bury work you do not. Both are Owner decisions — never abandon a packet without the Owner's explicit call.
