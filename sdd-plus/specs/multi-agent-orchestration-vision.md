...
- **Q3 coordination — YES, no human relay required.** Codex runs headless (`codex exec --json`), with **schema-constrained output** (`--output-schema` + `--output-last-message` → structured JSON back to a caller). And `codex mcp-server` exposes `codex(prompt, cwd?, model?, ...)` + `codex-reply(prompt, threadId)` as MCP tools — **Claude Code, being an MCP client, can call Codex directly.** Plus an experimental local app-server WebSocket listener using an unencrypted transport in the spike. **The conductor model is buildable: one side dispatches to the other; git/files carry artifacts; the human is no longer the mandatory bus.**
