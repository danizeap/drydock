...
The Owner surface: `scripts/brief.py` (plugin-only deterministic FACTS engine), the `/drydock:brief` command (translate-only plain-language rendering + the OWNER_STATUS.md snapshot), and the per-user event ledger the guard hooks feed. The design's death condition is false peace of mind — every fact is computed by code from the project's own records; the model may translate, never embellish; absence renders as unavailable, never as fine. Unit of the surface: the commitment — each work item is a rung on a fixed promise ladder captioned with what the Owner can safely say out loud.
