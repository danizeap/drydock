...
`scripts/release.py`, the maintainer release helper. It keeps Drydock's version declarations in lockstep, detects drift, gates releases behind the test suite, and prepares — but never executes — the publish commands. Dev-only tooling; not shipped in the project scaffold.
