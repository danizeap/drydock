...
The LaunchGuardian security-shipping layer. The canonical gate detail, severity model, and skip rules live in `sdd-plus/specs/launchguardian-framework.md` (single source of truth); the recordable applicability artifacts live under `sdd-plus/security/`. This living capability spec captures durable, testable requirements about how gates are assessed — starting with the Gate 15 lethal-trifecta test. It does not duplicate the framework spec's gate table.
