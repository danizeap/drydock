...
`hooks/completion_gate.py`, the `Stop` hook that makes "done" mean *verified* done. When a change packet looks claimed-done but its verification is still Pending and it changed this session, it blocks the stop once with a precise nudge. Read-only project scan; ships with the plugin; fires at every turn end. Loop-safety is paramount — the fail direction is always silent-allow (the archive gates remain the deterministic backstop).
