...
How the agent asks a human for approval, and how a decline is handled. The operative protocol and the trigger list live in `sdd-plus/protocols/framework-usage.md` §7 (single source of truth); the fillable forms live in `sdd-plus/templates/approval-request.md`. This living capability spec captures the durable, testable requirements. The deterministic enforcement layer (a per-packet `approvals.md` record and an archive gate) is intentionally not part of this capability yet — it is a separate, red-teamed change.
