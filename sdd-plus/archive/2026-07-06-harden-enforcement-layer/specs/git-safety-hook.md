...
First living spec for the destructive-git PreToolUse guard (`hooks/git_safety.py`). The hook reads the Bash tool-call JSON on stdin and exits 2 (block, reason on stderr) or 0 (allow).
