...
First living spec for `hooks/session_orient.py`, the `SessionStart` hook that orients the agent to Drydock project state and probes guardrail liveness. Read-only; ships with the plugin; fires in every session.
